...
| **Reward** | — | Điểm số phản hồi sau mỗi bước (vd nhập làn +50 một lần + thưởng-tiến-độ mỗi bước, va chạm −100). |
...
│  • Đường cao tốc 3 làn (240m) + nhánh nhập làn (ramp)      │
...
**15D của `highway_0/1/2`** (nguồn: `get_current_state`): tốc độ, làn hiện tại, và radar 5 hướng (cùng-làn-trước; trái trước/sau; phải trước/sau — mỗi hướng khoảng cách+tốc độ) + xe ramp đang merge + chỉ số kiên nhẫn.
...
| Thành phần | Giá trị |
|---|---|
| Nhập làn thành công (một lần khi merge) | **+50** |
| Tiến độ dense (potential-based, mỗi tick) | +k×Δ(x/đích), k=200 |
...
| Terminal va chạm | −100 |
| Base mỗi tick (giữ tốc) | speed×0.04 + action_penalty |
| Thưởng dòng chảy (flow on) | +speed×0.06; +0.05 nếu tốc≥0.7×max |
| Tốc độ quá thấp (<0.2×max) | −0.05 |
| TTC penalty (gap<20m) | −((20−gap)/20)×speed×1.3 |
| Action 0 (tăng tốc) khi gap<18m | −0.8 |
| Phạt phanh vô cớ (giảm tốc không có merger gần) | −0.6 |
| Cooperative bonus (nhường merger đang gần) | +0.3 ~ +0.4 |
| Phạt khiên tỷ lệ | −shield_prop_coef×clip, k=5 |
...
- Đường: `number_of_lanes=3`, `lane_width=3.5m`, `road_length=240m` (kéo dài từ 200m để đo sóng lùi/e2e sau điểm nhập).
...
- Mật độ: `nb_cars_max` (24 ở scenario `low`, 54 medium, 84 high — giữ nguyên mật độ/đơn-vị khi đường dài 240m).
...
| **`baselines.py`** | Chính sách **không học** để so sánh: `greedy` (tham lam — tăng tốc + merge sớm + vượt làn; hỗ trợ `--rl-postmerge` để tự lái e2e và `--no-shield` cho ablation, cùng cơ chế với RL). *(`random` và `base_rule` đã bỏ — demo GUI Heuristic dùng luật phía GAML.)* |
...
| **`scenario_utils.py`** | Vá GAML theo scenario (regex thay `nb_cars_max`, `spawn_ramp_tick`) + `set_safety_shield(bool)` bật/tắt cờ `enable_safety_shield` cho A/B khiên; khôi phục bằng `git checkout` (không dùng .bak — tránh mồ côi đè mất code). |
| **`run_experiments.py`** | **Điều phối toàn pipeline**: baseline (greedy) → train (PPO+A2C × nhiều seed) → select-best-checkpoint → eval → plots → analysis. Cờ `--scenario low/medium/high`, `--shield on/off`, `--run-tag` (tách output). |
...
| Learning rate | 1e-4 | 7e-4 (thesis; mặc định code 3e-4) |
| `n_steps` | 256 | 256 |
...
| `normalize_advantage` | True (mặc định PPO) | True (bật tay) |
...
run_experiments.py --preset thesis --scenario <low|medium|high> --shield <on|off>
   ...
   └─ restore GAML bằng git checkout (khôi phục bản committed)
```
...
**Các cơ chế giai đoạn 2 (end-to-end):**

- **Grace-die (GAML `die_if_out_of_road`)**: xe RL tới cuối đường KHÔNG `do die` cùng chu kỳ — giữ 2 chu kỳ (`dead_timer`) cho tầng thu thập GAMA-Python đọc termination + reward terminal. Thiếu cơ chế này, tín hiệu kết-thúc-episode bị nuốt, critic không học được (ev xấp xỉ 0) và mọi lần train e2e đều sụp. Công cụ chẩn đoán: `rl/probe_reward_stream.py` (in dòng reward/done thô SB3 nhận).
- **Dense progress reward** (`calculate_merging_reward`): cộng `merge_progress_coef × Δ(x/exit)` mỗi tick (potential-based, Ng 1999) + merge bonus một lần (+50).
- **Phạt khiên tỷ lệ** (`shield_prop_coef=5`): trừ `k × (ga RL − ga khiên cho phép)` mỗi tick — thay phạt nhị phân -1 chỉ kích pha khẩn; counter `shield_intervention_count` xuất metric "ỷ-khiên" qua `get_episode_info`.
- **Cờ eval**: `pz_eval_continue`/`pz_postmerge_window`/`pz_rl_postmerge` (e2e) + `pz_no_shield` (ablation bỏ IDM-cap dọc lẫn gate ngang) — bơm từ Python qua `reset_marl_episode`.
- **`train_curriculum.sh`**: 2 stage from-scratch (terminate-at-merge → e2e warmstart) tái lập model thesis không sửa code; `rl/make_thesis_plots.py` sinh 7 biểu đồ PNG cho báo cáo.
...
**Các loại khiên an toàn (ghi đè hành động, áp đồng đều RL + baseline):**
1. **IDM-cap dọc** (`applied = min(rl_acc, idm_safe)`, áp merger sau-merge + 3 highway) — chống đâm đuôi.
2. **Khiên ngang** (cổng đổi làn, gaml ~2247) — chặn lane-change khi gap trái/phải < ngưỡng (8 m/6 m).
3. **Cổng nhập làn** `is_merge_gap_safe` — *ĐÃ TẮT* (`commit_gate_real ← false`, 2026-06-07) cho mọi policy để greedy↔RL công bằng tại pha nhập làn; hàm vẫn tính gap cho observation/reward/dashboard nhưng không chặn hành động.

**Hai cờ tắt khiên cho 2 thực nghiệm:** (a) `enable_safety_shield` (giai đoạn 1, A/B) — `--shield off` vá `false` cho cả run headless (GUI Heuristic luôn giữ khiên nhờ `dashboard_policy`). (b) `pz_no_shield` (giai đoạn 2, ablation eval-only) — tắt **IDM-cap dọc + khiên ngang** để đo nội-tâm-hóa an toàn. Đều là công cụ tách bạch "năng lực nội tại" với "trợ giúp môi trường" (xem §8).
...
> **LƯU Ý PHẠM VI:** bảng dưới là **giai đoạn 1** (kết-thúc-tại-điểm-nhập, đường 200m, đánh giá LẤY MẪU). Kết quả end-to-end giai đoạn 2 (đường 240m, 3 hạt giống, PPO argmax 86±6% vs Greedy 53±12%) + hiện tượng deterministic-collapse của A2C xem DO_AN_TOT_NGHIEP §4.6.
>
> **Đọc kết quả giai đoạn 1:** (1) **MAA2C** cao + ổn định nhất mọi cấu hình (std 1.6–5.2), vượt MAPPO (std nổ tới ±23 khi không khiên) — ĐO Ở CHẾ ĐỘ LẤY MẪU; ở chế độ tất định (deploy) thứ hạng đảo (xem §4.6.4 DO_AN). (2) **A/B khiên** là phát hiện chính: Greedy có khiên 96% nhưng gỡ khiên sụp theo mật độ (82→2→0%) → "năng lực" của nó là vay mượn từ lưới an toàn; RL nội hóa an toàn nên bền (MAA2C 86→87, 89→85, 83→85%). (3) Cùng điều kiện không-khiên, RL thắng Greedy áp đảo (medium 85% vs 2%). Eval **stochastic** (sampling) cho số thực tế có phương sai; deterministic (argmax) cho số cao hơn nhưng dễ ảo.
...
- **Ablation khiên tắt 2 cơ chế chủ động** (IDM-cap dọc + khiên ngang đổi làn); cổng nhập làn đã tắt sẵn cho mọi policy. Các cơ chế chống-kẹt/hồi-sinh (anti-deadlock, respawn, kick-start) luôn bật cho mọi policy → công bằng nhưng không phải môi trường "trần trụi" hoàn toàn.
