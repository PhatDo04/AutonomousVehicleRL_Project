...
global {
    ...
    int   nb_cars_max          <- 45;   // ← tăng từ 20 lên 45
    ...
    reflex spawn_ramp_cars when: every(40 #cycles) {
        if (flip(0.5)) {
    ...
    reflex balance_traffic when: every(15 #cycles) {
        ...
        if (total_cars < nb_cars_max) {
            ...
            bool is_safe   <- empty(car where (each.current_lane_index = spawn_lane and each.location.x < 25.0));
            if (is_safe) {
                create car number: 1 {
    ...
    // 45/3 = 15 xe/làn, spacing = 200/16 ≈ 12.5 units
        ...
        loop i from: 0 to: number_of_lanes - 1 {
            loop j from: 0 to: cars_per_lane - 1 {
                create car number: 1 {
                    ...
                    target_lane_index  <- i;
                    location           <- {10.0 + j * spacing, offset_y + (i * lane_width) + (lane_width / 2.0)};
                    direction          <- 1;
                    heading            <- 0.0;
                }
            }
...
species car skills: [moving] {
    ...
    action ramp_behavior {
        ...
        car ramp_ahead <- (car - self) where (each.is_merging and each.location.x > self.location.x)
                          with_min_of (each.location.x - self.location.x);
        if (ramp_ahead != nil) {
            float dist <- location distance_to ramp_ahead.location;
            if (dist < car_length + 1.5) {
        ...
        if (in_accel_zone) {
            ...
            if (gap_ok or stuck_counter > 150) {
                do execute_merge();
                return;
            }
    ...
    car ahead_final <- get_car_ahead();
    if (ahead_final != nil) {
        float dist_final <- ahead_final.location.x - self.location.x;
        if (dist_final < car_length + 0.3) {
            speed <- 0.0;
        } else if (dist_final < car_length * 2.5) {
            speed <- min(speed, (dist_final - car_length) * 0.4);
        }
    }

    // ── Di chuyển chéo mượt mà (chỉ 1 block duy nhất) ──
    ...
    action get_car_ahead type: car {
        list<car> ahead_cars <- (car - self) where (
            ...
            each.location.x > self.location.x + 0.1
        );
    ...
    action get_safe_distance type: float {
        if (speed <= 0.6) { return 4.0; }
        else if (speed <= 0.8) { return 6.0; }
        else { return 9.0; }
    }
